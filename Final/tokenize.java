...
public class tokenize {
    public static class TkMapper extends Mapper<Object, Text, Text, IntWritable> {
        ...
        @Override
        // Output　key is term##filename: frequency
        protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if(SplitedLine != null) {
                for(String s: SplitedLine) {
                    if(!stopwords.contains(s)) {// remove the stopwrods
    ...
    public static class TkReducer extends Reducer<Text, IntWritable, Text, Text> {
        ...
        private int threshold = 2;// words less than 2 will not be contained in dictionary
        
        @Override
        // Output: term totalnum    file1:num1,file2:num2
        protected void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
            String cur_term = key.toString().split("##")[0];
            String cur_file = key.toString().split("##")[1];
